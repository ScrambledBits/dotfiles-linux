...
if [[ -n "${HOMEBREW_ON_LINUX-}" && (-n "${HOMEBREW_CURL_PATH-}" || -n "${HOMEBREW_GIT_PATH-}") ]]; then
...
EOABORT
  ...
  elif version_lt "${macos_version}" "10.11"; then
...
if [[ -n "${HOMEBREW_INSTALL_FROM_API-}" ]]; then
...
if [[ -n "${HOMEBREW_ON_MACOS-}" ]] && ! output="$(/usr/bin/xcrun clang 2>&1)" && [[ "${output}" == *"license"* ]]; then
  abort "$(
    cat <<EOABORT
